...
TARGET_HZ = 35.0   # canonical main-model frequency, applied to the preliminary all-to-all model
SIGMA_HZ = 2.0
...
def simulate():
    ...
    for t in range(T):
        ...
        coupling_sum = np.sum(np.sin(diff), axis=1)
...
def plot(da, K, R, lfp):
    ...
    ax2.set_ylabel("DA, R")
    ax2.set_ylim(0, 1.05)
    ax1.set_title("(A) K(t), DA, R(t), all-to-all, 35 Hz (N=200)")
    ...
    plt.tight_layout()
    plt.savefig("Figure1b_AllToAll35Hz.png", dpi=300, bbox_inches="tight")
    plt.close()
...
if __name__ == "__main__":
    ...
    plot(da, K, R, lfp)
    half = T // 2
    dom = dominant_freq(lfp[half:], DT)
    print(f"Final R = {R[-1]:.3f}, K max = {K.max():.3f}, dominant late freq = {dom:.2f} Hz")
    print("Saved Figure1b_AllToAll35Hz.png")
